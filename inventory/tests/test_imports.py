...
class BulkImportTests(TestCase):
    def setUp(self):
        self.admin = User.objects.create_user(
            "NXTTP0001", "Admin User", "+919876543210", "StrongPass123!", role=User.Role.ADMIN
        )

    def test_employee_import_creates_account_and_reports_duplicate(self):
        file = workbook_file(
            ["employee_id", "full_name", "mobile_number", "default_tshirt_size"],
            [
                ["NXTTP0043", "Employee One", "+919876543211", "L"],
                ["NXTTP0043", "Duplicate", "+919876543212", "M"],
            ],
        )
        result = import_employees(file, self.admin)
        self.assertEqual(result.created, 1)
        self.assertEqual(result.failed, 1)
        self.assertTrue(User.objects.get(employee_id="NXTTP0043").must_change_password)

    def test_book_import_generates_asset_id(self):
        file = workbook_file(
            ["book_name", "class_name", "condition"],
            [["Physics", "11", "GOOD"]],
        )
    ...
    def test_tshirt_import_updates_stock_and_purchase_history(self):
        file = workbook_file(
            ["brand", "size", "quantity", "free_allowance", "low_stock_threshold"],
            [["Next Toppers", "L", 25, 5, 4]],
        )
